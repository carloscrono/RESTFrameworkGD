...
open class SessionManager {
  ...
  open static let defaultSession: SessionManager = {
  ...
  open static let defaultHTTPHeaders: HTTPHeaders = {
    ...
    let userAgent: String = {
      if let info = Bundle.main.infoDictionary {
        ...
        let osName = "iOS"
        let osNameVersion: String = "\(osName) \(versionString)"
        
        let afInfo = Bundle(for: SessionManager.self).infoDictionary
        let build = afInfo!["CFBundleShortVersionString"]
        let swiftyRESTVersion: String = "SwiftyREST/\(String(describing: build))"
    ...
    DispatchQueue.global(qos: .utility).async {
      ...
      do {
        ...
        if formData.contentLength < encodingMemoryThreshold && !isBackgroundSession {
          ...
          self.lintingUpload(formData: formData, urlRequestWithContentType: urlRequestWithContentType, encodingCompletion: encodingCompletion!)
        }
      } catch {
  ...
  func lintingUpload(formData: MultipartFormData, urlRequestWithContentType: URLRequest, encodingCompletion: @escaping ((MultipartFormDataEncodingResult) -> Void)) {
    var tempFileURL : URL?
    do {
  ...
  private func allowRetrier(_ retrier: RequestRetrier, toRetry request: Request, with error: Error) {
    DispatchQueue.utility.async { [weak self] in
      ...
      self?.allowingRetry(retrier: retrier, request: request, error: error, strongSelf: strongSelf)
    }
  }
  
  func allowingRetry(retrier: RequestRetrier, request: Request, error: Error, strongSelf: SessionManager) {
    
    retrier.should(strongSelf, retry: request, with: error) { shouldRetry, timeDelay in
      let strongSelf = self
      
      guard shouldRetry else {
        if strongSelf.startRequestsImmediately {
          request.resume()
        }
        return
      }
